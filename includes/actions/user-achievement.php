...
class AutomatorWP_GamiPress_User_Achievement extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Award achievement to user', 'automatorwp-gamipress' ),
            'select_option'     => __( 'Award <strong>achievement</strong> to user', 'automatorwp-gamipress' ),
            /* translators: %1$s: Post title.  %2$s: User. */
            'edit_label'        => sprintf( __( 'Award the achievement %1$s to %2$s', 'automatorwp-gamipress' ), '{post}', '{user}' ),
            /* translators: %1$s: Post title. %2$s: User. */
            'log_label'         => sprintf( __( 'Award the achievement %1$s to %2$s', 'automatorwp-gamipress' ), '{post}', '{user}' ),
            'options'           => array(
                'post' => automatorwp_gamipress_utilities_post_option( array(
                    'name' => __( 'Achievement:', 'automatorwp-gamipress' ),
                    'option_default' => __( 'Select an achievement', 'automatorwp-gamipress' ),
                    'option_none' => false,
                    'option_custom'         => true,
                    'option_custom_desc'    => __( 'Achievement ID', 'automatorwp-gamipress' ),
                ...
                'user' => array(
                    ...
                    'fields' => array(
                        'user' => array(
                            ...
                            'desc' => __( 'User ID that will receive this achievement. Leave blank to award the achievement to the user that completes the automation.', 'automatorwp-gamipress' ),
                            'type' => 'input',
                            'default' => ''
                        ),
                    )
